...
public class CustomizedSquareFlowLayout: SquareFlowLayout {
    @IBInspectable public var titleHeaderSectionHeight : CGFloat = 0.0
    ...
    public override func prepare() {
        ...
        for section in 0 ..< collectionView.numberOfSections {
            ...
            for item in 0 ..< collectionView.numberOfItems(inSection: section) {
                layoutValues.append(flowDelegate?.shouldExpandItem(at: IndexPath(row: item, section: section)) == true)
            }
            let chunkSize = 3
            ...
            let contentWidth = layouts.count == 1 ? collectionView.frame.width - (collectionView.contentInset.right + collectionView.contentInset.right) : expandedWidth
            ...
            for layout in layouts {
                ...
                if section == 0 || section == 2 {
                    ...
                    case .start:
                        add(rect: CGRect(x: 0, y: yOffset, width: contentWidth, height: titleHeaderSectionHeight), at: 0, in: layout)
                        add(rect: CGRect(x: interSpacing, y: yOffset, width: itemWidth, height: titleHeaderSectionHeight), at: 1, in: layout)
                        add(rect: CGRect(x: interSpacing, y: titleHeaderSectionHeight, width: itemWidth, height: titleHeaderSectionHeight), at: 2, in: layout)
                    case .none:
                        for i in 0 ..< layout.count {
                            add(rect: CGRect(x: xOffset, y: yOffset, width: layouts.count == 1 ? contentWidth : itemWidth, height: titleHeaderSectionHeight), at: i, in: layout)
                    ...
                    xOffset = 0
                    yOffset = yOffset + titleHeaderSectionHeight + interSpacing
                } else {
                    
                    switch expandedPosition {
                    case .start:
                        add(rect: CGRect(x: 0, y: yOffset, width: contentWidth, height: expandedHeight), at: 0, in: layout)
                    ...
                    case .none:
                        for i in 0 ..< layout.count {
                            add(rect: CGRect(x: xOffset, y: yOffset, width: layouts.count == 1 ? contentWidth : itemWidth, height: itemHeight), at: i, in: layout)
                            xOffset = xOffset + itemWidth + interSpacing
                        }
                    }
                    xOffset = 0
